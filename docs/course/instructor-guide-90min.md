...
### スライド
（スライドはタイトルで参照。番号はデッキ再生成でズレるため使わない）
- スライド「中心メッセージ」
- スライド「§1. こんな経験ありませんか?」
- スライド「Harness Engineering の原典」
- スライド「プロンプトエンジニアリングとの違い」
...
### スライド
- スライド「§6. 比較デモの設計」
- スライド「Round 1: ハーネスなし」「Round 2: ハーネスあり」
- スライド「比較サマリ」「観察 → engineer-out 体験」「中心メッセージ (再)」
...
2. Claude Code を起動 — **`claude --model claude-sonnet-4-6` でモデルを Round 2 と揃える**（no-harness コピーは settings.json を消しているため、フラグ無しだと個人既定モデルになり「AI モデルも同じ」が崩れる。詳細は demo 手順書）
...
### スライド
- スライド「§7. Phase 1〜5 ロードマップ」
- スライド「持ち帰り宿題」「自社導入チェックリスト」「1 行に圧縮」
...
- JDK が **17 または 21 (LTS)** か確認 (`java -version`)。**24+ は Gradle 8.10 が起動できない**ので不可（「17 以上」では NG。participant-guide と同じ基準）
